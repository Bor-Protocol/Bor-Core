...
import {
    ...
    formatActionNames,
    formatActions,
} from "./tobeadded/actions.ts";
import { addHeader, composeContext } from "./utils/context.ts";
...
import { formatGoalsAsString, getGoals } from "./tobeadded/goals.ts";
import { aiKhwarizmiLogger, embed, splitChunks } from "./index.ts";
import { embeddingZeroVector, MemoryManager } from "./memory.ts";
import { formatActors, formatMessages, getActorDetails } from "./messages.ts";
import { parseJsonArrayFromText } from "./utils/parsing.ts";
import { formatPosts } from "./posts.ts";
import { getProviders } from "./tobeadded/providers.ts";
...
export class AgentRuntime implements IAgentRuntime {
    ...
    registerAction(action: Action) {
        aiKhwarizmiLogger.success(`Registering action: ${action.name}`);
    ...
    async processActions(
        ...
        if (!responses[0].content?.action) {
            aiKhwarizmiLogger.warn("No action found in the response content.");
            return;
        }
        ...
        aiKhwarizmiLogger.success(`Normalized action: ${normalizedAction}`);
        ...
        if (!action) {
            aiKhwarizmiLogger.info("Attempting to find action in similes.");
        ...
        if (!action.handler) {
            aiKhwarizmiLogger.error(`Action ${action.name} has no handler.`);
            return;
        }

        aiKhwarizmiLogger.success(`Executing handler for action: ${action.name}`);
...
*/
    ...
    async ensureUserExists(
        ...
        if (!account) {
            ...
            aiKhwarizmiLogger.success(`User ${userName} created successfully.`);
        }
    }
    ...
    async ensureRoomExists(roomId: UUID) {
        ...
        if (!room) {
            ...
            aiKhwarizmiLogger.log(`Room ${roomId} created successfully.`);
        }
    }
